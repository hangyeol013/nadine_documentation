...
|---|---|---|---|
| **Multi-Agent, Local** | LangGraph state graph with ten specialized agents ([Agents & Graph](interaction-agents.md)) | Fully on-device via Ollama: fine-tuned Qwen2.5-1.5B models (LoRA adapters merged and exported to GGUF), Mistral Small 3.2, Qwen2.5-VL for vision | User profile, episodic and visual memory in ChromaDB and CLIP ([Memory & RAG](interaction-memory-rag.md)); PAD affect model with per-turn appraisal and update; no per-turn API cost. Used for agent development, fine-tuning, and offline operation. |
| **Multi-Agent, Hybrid Cloud** | Same graph, with intent, affect and planning merged into one turn-understanding call ([Cloud Configuration](interaction-cloud-config.md)) | Hybrid: gpt-5.4-mini for turn understanding, response, memory extraction and routing; vision description local; the fine-tuned local adapters are used only in the optional legacy pipeline mode | Adds observation scenes that pair user utterances with what the camera saw ([Multimodal Memory](interaction-multimodal-memory.md)), A/B study mode and logging; PAD affect model with appraisal folded into turn understanding. Used for multimodal memory research and user studies. |
| **ReAct, Cloud** | Contextualizer, memory and knowledge retrieval, then a LangChain ReAct agent with a fixed tool set | Cloud only: gpt-5.4-mini and gpt-4o-mini via OpenAI | Per-user long-term conversation memory in ChromaDB plus a knowledge base, both retrieved on the contextualized question; per-turn self-appraised emotion with intensity and cause, no persistent affect state; barge-in with lip-sync recovery and a singing pipeline. Used for public demonstrations and recorded sessions. |
