...
| Version | Dialogue engine | Language models | Memory | Affect | Primary use |
|---|---|---|---|---|---|
| **Multi-Agent, Local** | LangGraph state graph with ten specialized agents ([Agents & Graph](interaction-agents.md)) | Fully on-device via Ollama: fine-tuned Qwen2.5-1.5B models (LoRA adapters merged and exported to GGUF), Mistral Small 3.2, Qwen2.5-VL for vision | User profile, episodic and visual memory in ChromaDB and CLIP ([Memory & RAG](interaction-memory-rag.md)) | PAD model with per-turn appraisal and update | Agent development, fine-tuning, offline operation |
| **Multi-Agent, Hybrid Cloud** | Same graph, with intent, affect and planning merged into one turn-understanding call | Hybrid: gpt-5.4-mini for turn understanding, response, memory extraction and routing; local adapters for the legacy nodes and episodic context; local vision | Adds observation scenes that pair user utterances with what the camera saw, and study logging | PAD model, appraisal folded into turn understanding | Multimodal memory research and user studies |
| **ReAct, Cloud** | Contextualizer, memory and knowledge retrieval, then a LangChain ReAct agent with a fixed tool set | Cloud only: gpt-5.4-mini and gpt-4o-mini via OpenAI | Per-user long-term conversation memory in ChromaDB plus a knowledge base, both retrieved on the contextualized question | Per-turn self-appraised emotion with intensity and cause in the structured answer; no persistent state | Public demonstrations and recorded sessions |
...
**Consequence.** Agents whose failures would corrupt the study data moved to gpt-5.4-mini: turn understanding, the response agent, memory extraction, and the search and vision routers. Merging intent, affect, and planning into one call also removed two sequential model calls from every turn. Extraction and episodic encoding run at most once per conversation, so the cloud cost of this change is negligible, and moving the routers off the GPU freed VRAM. Episodic contextualization stayed on a local adapter, and the original local nodes remain selectable.
